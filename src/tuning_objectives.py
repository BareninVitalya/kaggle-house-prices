...
from .evaluate import cv_scores_with_fit_params
...
class RidgeObjective:
    ...
    def __call__(self, trial):
        params = {
            f"{self.step_name}__alpha": trial.suggest_float(
                "model__alpha", 1e-4, 100.0, log=True
            ),
            f"{self.step_name}__fit_intercept": trial.suggest_categorical(
        ...
        if self.log_trials:
            _log_optuna_trial(trial, scores, self.X, "ridge", self.logfile)
...
class LassoObjective:
    ...
    def __call__(self, trial):
        params = {
            f"{self.step_name}__alpha": trial.suggest_float(
                "model__alpha", 1e-5, 10.0, log=True
            ),
            f"{self.step_name}__fit_intercept": trial.suggest_categorical(
                "model__fit_intercept", [True, False]
            ),
            f"{self.step_name}__max_iter": trial.suggest_int(
                "model__max_iter", 1000, 10000, step=1000
            ),
        }
        ...
        if self.log_trials:
            _log_optuna_trial(trial, scores, self.X, "lasso", self.logfile)

        return float(scores.mean())

class ElasticNetObjective:
    def __init__(
        self,
        base_model,
        X,
    ...
    def __call__(self, trial):
        params = {
            ...
            f"{self.step_name}__l1_ratio": trial.suggest_float(
        ...
        if self.log_trials:
            _log_optuna_trial(trial, scores, self.X, "elasticnet", self.logfile)
...
class TreeObjective:
    ...
    def __call__(self, trial):
        params = {
            f"{self.step_name}__criterion": trial.suggest_categorical(
                "model__criterion",
            ...
            f"{self.step_name}__min_samples_split": trial.suggest_int(
                "model__min_samples_split", 2, 40
            ),
            f"{self.step_name}__min_samples_leaf": trial.suggest_int(
...
class RFObjective:
    ...
    def __call__(self, trial):
        params = {
            ...
            f"{self.step_name}__bootstrap": trial.suggest_categorical(
                "model__bootstrap",
                [True, False]
            ),
            f"{self.step_name}__random_state": SEED,
            f"{self.step_name}__n_jobs": -1,
        }
...
class CatBoostObjective:
    ...
    def __call__(self, trial):
        ...
        mean, std, scores, train_mean = cv_scores_with_fit_params(
            build_model,
            'cat',
            self.X,
            self.y,
