...
class HouseMLP(nn.Module):
    """
    Гибкая MLP для регрессии (House Prices).

    output_dim=1 — предсказываем одну непрерывную величину (лог-цена).
    """
    ...
    def __init__(
        ...
        # последний линейный слой без активации — регрессия
...
def _get_regression_loss_fn(name: str):
    name = name.lower()
    if name in ("mse", "l2"):
        return nn.MSELoss()
    if name in ("mae", "l1"):
    ...
    if name == "rmse":
        return nn.MSELoss()
    raise ValueError(
        f"Неизвестная loss '{name}'. Доступны: mse, mae, rmse"
    )


class HouseDNNRegressor(RegressorMixin, BaseEstimator):
    """
    Sklearn‑совместимый wrapper над HouseMLP для регрессии House Prices.

    Работает с cross_val_score, Pipelines и RandomizedSearchCV.
    """
    ...
    def __init__(
        ...
        loss_fn: str = "mse",      # mse / mae / rmse
        random_state: int = SEED,
        verbose: bool = False,
        val_size: float = 0.0,     # опционально будем делать валидацию
    ):
    ...
    def _evaluate(self, X_t, y_t, model, loss_fn):
        ...
        with torch.no_grad():
            ...
            loss = loss_fn(preds, y_t).item()
        y_np = y_t.cpu().numpy()
        p_np = preds.cpu().numpy()
        rmse = np.sqrt(mean_squared_error(y_np, p_np))
        r2 = r2_score(y_np, p_np)
        return loss, rmse, r2
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        loss_fn = _get_regression_loss_fn(self.loss_fn)
        ...
        for epoch in range(self.epochs):
            ...
            tr_loss, tr_rmse, tr_r2 = self._evaluate(
                X_tr_t, y_tr_t, self.model_, loss_fn
            )
            self.history_["train_loss"].append(tr_loss)
            self.history_["train_rmse"].append(tr_rmse)
            self.history_["train_r2"].append(tr_r2)

            if X_val_t is not None:
                val_loss, val_rmse, val_r2 = self._evaluate(
                    X_val_t, y_val_t, self.model_, loss_fn
                )
                self.history_["val_loss"].append(val_loss)
                self.history_["val_rmse"].append(val_rmse)
                self.history_["val_r2"].append(val_r2)
            ...
            if self.verbose:
                msg = (
                    ...
                    f"| train_rmse={tr_rmse:.4f} r2={tr_r2:.4f}"
                )
                if X_val_t is not None:
                    msg += (
                        f" | val_rmse={val_rmse:.4f} r2={val_r2:.4f}"
                    )
                print(msg)
    ...
    def score(self, X, y):
        # по умолчанию sklearn-score = R^2; можно переопределить на -RMSE,
        # если хочешь, чтобы cv совпадал с метрикой соревнования
        y_true = self._to_numpy(y) if hasattr(y, "values") else np.asarray(y)
