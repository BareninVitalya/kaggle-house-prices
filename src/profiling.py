...
class DataProfiler:
    ...
    def get_col_type(self, col: str) -> str:
        ...
        if is_numeric_dtype(dtype):
            ...
            if n_unique <= NUMERIC_AS_CATEGORICAL_MAX_UNIQUE and series.dropna().isin([0, 1]).all():
    ...
    def feature_mutual_info(self, X: pd.DataFrame, y: pd.Series) -> pd.Series:
        ...
        mi = mutual_info_regression(X_ready, y, discrete_features=discrete_features, random_state=SEED) # mutual_info_regression
    ...
    def feature_ablation(
        ...
        ablation_model = model(model_name, X_drop, transform_off=transform_off)
        drop_mean, *_ = cv_scores(ablation_model, X_drop, y, n_splits=cv, scoring=scoring)
    ...
    def feature_ablation_all(
        ...
        base_model = model(model_name, X, transform_off=transform_off)
        base_model_score, *_ = cv_scores(base_model, X, y, n_splits=cv, scoring=scoring)

        for f in X.columns:
            deltas[f] = self.feature_ablation(model, base_model_score, X, y, model_name=model_name, feature=f, cv=cv, scoring=scoring, transform_off=transform_off)
    ...
    def feature_report(
        ...
        """
        report = {}
        mutual_model = model(model_name, X, transform_off=transform_off)
        ...
        report["ablation"] = self.feature_ablation_all(model, X_fe, y, model_name=model_name, cv=cv, scoring=scoring, transform_off=transform_off)
        #
    ...
    def plot_training_history_both(self, history, title_prefix="Fold 1"):
        ...
        # справа — accuracy
        ax = axes[1]
        ax.plot(epochs, history["train_rmse"], label="train_rmse", color="tab:blue")
        ax.plot(epochs, history["val_rmse"], label="val_rmse", color="tab:orange")
        ax.set_title(f"{title_prefix}: rmse")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("rmse")
        ax.grid(True, alpha=0.3)
        ax.legend()
    ...
    def plot_training_history_both2(self, history, title_prefix="Fold 1",
                                   zoom_epochs=15, show_best=True):
        epochs = np.arange(1, len(history["train_loss"]) + 1)
        best_epoch = int(np.argmin(history["val_loss"]) + 1)

        # увеличиваем масштаб фигуры
        ...
        # логарифмическая шкала — лучше видно спад с 120 до мелких значений
        ax.set_yscale("log")

        # приближаем первые эпохи, где идёт основное падение
        ...
        ax.set_title(f"{title_prefix}: loss", fontsize=14)
        ...
        ax.set_title(f"{title_prefix}: RMSE", fontsize=14)
        ax.set_xlabel("Epoch", fontsize=12)
