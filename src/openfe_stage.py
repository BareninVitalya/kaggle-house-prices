...
class OpenFEStage:
    ...
    def _to_score(self, mean: float) -> float:
        """
        Если метрика «меньше — лучше» (RMSE), инвертируем знак
        """
        return mean if self.greater_is_better else -mean
    ...
    def _greedy_select(
        ...
        current_score, *_ = cv_scores(
            build_model(self.model_name, current_X, transform_off=True), current_X, y, scoring=self.scoring
        )
        current_score = self._to_score(current_score)
        selected: list[str] = []
        ...
        for col in ofe_cols:
            ...
            mean, std, *_ = cv_scores(
                build_model(self.model_name, cand_X, transform_off=True), cand_X, y, scoring=self.scoring
            )
            mean = self._to_score(mean)
            per_feat.append({
    ...
    def _ablation(
        ...
        base_score, base_std, *_ = cv_scores(
            build_model(self.model_name, X_base, transform_off=True), X_base, y, scoring=self.scoring
        )
        base_score = self._to_score(base_score)
        ...
        for n in range(self.ablation_step, len(ofe_cols) + 1, self.ablation_step):
            ...
            mean, std, *_ = cv_scores(
                build_model(self.model_name, X_exp, transform_off=True), X_exp, y, scoring=self.scoring
            )
            mean = self._to_score(mean)
            rows.append({
