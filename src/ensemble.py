...
from sklearn.linear_model import LinearRegression, LogisticRegression, RidgeClassifier, RidgeCV, Ridge, ElasticNet
...
def _build_submission(
        ...
        ids: pd.Series | pd.DataFrame,
) -> pd.DataFrame:
    """Собрать submission DataFrame для House Prices из Id и предсказаний."""
    ...
    return pd.DataFrame(
        {
            "Id": ids.squeeze().values,
            "SalePrice": np.expm1(predictions),
        }
    )


class SoftEnsemble(RegressorMixin, BaseEstimator):
    ...
    def predict(self, X):
        # взвешенное среднее регрессионных предсказаний
        total_weight = sum(self.weights.values())
        y_pred = 0
        for name in self.model_names:
            model = self.models_[name]
            y_pred += self.weights[name] * model.predict(X)
    ...
    def predict_test(
        ...
        """Сформировать submission DataFrame для тестового набора House Prices."""
...
class StackingEnsembleRegressor:
    """Кастомная обёртка для stacking-регрессии с OOF-признаками."""
    ...
    def _make_meta_model(self):
        """Создать мета-модель регрессии по имени."""
        if self.meta_model_name == "ridge":
            return Ridge(**self.meta_params)

        if self.meta_model_name == "elastic":
            return ElasticNet(**self.meta_params)

        if self.meta_model_name == "cat":
            return modeling.build_model(
                "cat",
                X=None,
    ...
    def build_oof_data(
        ...
        """Построить OOF-признаки для мета-модели и мета-признаки теста."""
        kf = KFold(
            n_splits=self.n_splits,
            shuffle=True,
            random_state=self.random_state,
        ...
        for model_name in self.model_names:
            ...
            test_fold_pred = np.zeros((len(X_test), self.n_splits), dtype=float)
            ...
            for fold, (tr_idx, val_idx) in enumerate(kf.split(X, y), start=1):
                ...
                oof_col[val_idx] = val_pred
                test_fold_pred[:, fold - 1] = test_pred_fold
    ...
    def build_meta_data(
        ...
        for model_name in self.model_names:
            ...
            fold_models = self.base_models_[model_name]
            fold_pred = np.zeros((len(X_new), len(fold_models)), dtype=float)

            for fold, model in enumerate(fold_models):
                fold_pred[:, fold] = model.predict(X_new)
    ...
    def fit(
        ...
        """Построить OOF-признаки и обучить мета-регрессор."""
        oof_pred, test_pred = self.build_oof_data(X=X, y=y, X_test=X_test)
    ...
    def predict(
        ...
        """Вернуть предсказания мета-модели."""
        if self.meta_model_ is None:
            raise ValueError("StackingEnsembleRegressor is not fitted yet.")

        X_meta = X if is_meta_data else self.build_meta_data(X)
        pred = self.meta_model_.predict(X_meta)
    ...
    def predict_test(
        ...
        if meta_features is None:
            raise ValueError(
                "Test meta-features are not available. Fit the ensemble first or pass X_meta_test."
            )

        pred = self.predict(meta_features, is_meta_data=True)
        ids = df_test[["Id"]]

        sale_price = np.expm1(pred) if self.use_expm1 else pred
